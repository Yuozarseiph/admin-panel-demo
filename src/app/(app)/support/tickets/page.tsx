...
export default function SupportTicketsInboxPage() {
  const [selectedId, setSelectedId] = useState<string>(ticketsData[0]?.id ?? "");
  ...
  const selected = ticketsData.find((t) => t.id === selectedId) ?? ticketsData[0];
  ...
  let filtered = ticketsData.filter((t) => {
    ...
    const byTab =
      tab === "همه"
        ...
        : t.channel === "چت";
    const byPriority = priorityFilter === "همه" ? true : t.priority === priorityFilter;
  ...
  if (sortMode === "جدیدترین") {
    ...
    const pMap: Record<Priority, number> = { فوری: 4, زیاد: 3, متوسط: 2, کم: 1 };
    ...
    <main dir="rtl" className="p-4 sm:p-6 text-[20px]">
      <section className="rounded-xl border border-gray-200 bg-white text-gray-800 shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-5 py-3 dark:border-gray-800">
          ...
          <Link href="/support/new" className="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-[16px] font-semibold text-white hover:bg-sky-700">
            <Plus size={18} />
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-gray-100 px-5 py-3 text-[14px] dark:border-gray-800">
          {([
            { key: "همه" as TicketTab, label: "همه", count: ticketsData.length },
            { key: "چت" as TicketTab, label: "چت", count: ticketsData.filter((t) => t.channel === "چت").length },
            { key: "برای من" as TicketTab, label: "اختصاص داده شده به من", count: ticketsData.filter((t) => t.assignedTo === "احمد رضایی").length },
            { key: "اختصاص‌داده‌نشده" as TicketTab, label: "تیکت‌های بدون مسئول", count: ticketsData.filter((t) => !t.assignedTo).length },
          ] satisfies { key: TicketTab; label: string; count: number }[]).map((item) => (
            <button
              key={item.key}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3 text-[14px] dark:border-gray-800">
          <div className="flex flex-wrap items-center gap-2">
            <Dropdown label="اولویت" value={priorityFilter} options={["همه", "فوری", "زیاد", "متوسط", "کم"]} onChange={(v) => setPriorityFilter(v as Priority | "همه")} />
            <Dropdown label="وضعیت" value={statusFilter === "همه" ? "همه" : statusFilter} options={["همه", "باز", "بسته"]} onChange={(v) => setStatusFilter(v as TicketStatus | "همه")} />
            <Dropdown label="انتخاب کارشناس" value={agentFilter} options={["همه", ...agents]} onChange={(v) => setAgentFilter(v)} />
          </div>
          <Dropdown label="مرتب‌سازی" value={sortMode} options={["جدیدترین", "قدیمی‌ترین", "اولویت بالا"]} onChange={(v) => setSortMode(v as SortMode)} />
        </div>
            ...
            <div className="flex items-start justify-between border-b border-gray-200 px-4 py-3 text-[14px] dark:border-gray-800">
              <div className="flex flex-col gap-1">
                <div className="inline-flex items-center gap-2">
                  <span className="rounded-full bg-rose-50 px-2 py-0.5 text-[11px] text-rose-600 dark:bg-rose-900/30 dark:text-rose-300">{selected.tag}</span>
                  <h2 className="text-[18px] font-semibold">{selected.title}</h2>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-gray-500">
                  <span className="inline-flex items-center gap-1"><UserCircle2 size={14} /><span>{selected.customerName}</span></span>
                  <span className="text-gray-400">•</span>
                  <span className="text-[12px]">{selected.customerEmail}</span>
                  <span className="text-gray-400">•</span>
                  <span className="text-[12px]">{selected.openedAt}</span>
                  <span className="text-gray-400">•</span>
                  <span className="inline-flex items-center gap-1 text-[12px]"><MessageCircle size={13} /><span>{selected.channel}</span></span>
                </div>
              </div>
              <span className={["rounded-full px-3 py-1 text-[12px]", selected.status === "باز" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-gray-100 text-gray-600 dark:bg-gray-800/60 dark:text-gray-300"].join(" ")}>
                {selected.status === "باز" ? "باز" : "بسته شده"}
              </span>
            </div>

            <div className="flex-1 overflow-y-auto px-6 py-5 text-[15px] leading-relaxed text-gray-700 dark:text-gray-200">{selected.body}</div>

            <div className="border-t border-gray-200 px-4 py-3 dark:border-gray-800">
              <label className="block text-right text-[13px] text-gray-600 dark:text-gray-300">
                پاسخ سریع به این تیکت
                <textarea value={reply} onChange={(e) => setReply(e.target.value)} className="mt-2 min-h-[90px] w-full rounded-lg border border-gray-300 px-3 py-2 text-[14px] leading-relaxed outline-none focus:border-sky-500 dark:border-gray-700 dark:bg-gray-950" placeholder="متن پاسخ را اینجا بنویس..." />
              </label>
              <div className="mt-3 flex items-center justify-between">
                <button className="inline-flex items-center gap-1 text-[13px] text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"><Tag size={14} /><span>افزودن تگ / یادداشت</span></button>
                <button type="button" onClick={sendReply} className="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-[14px] font-semibold text-white hover:bg-sky-700"><Send size={16} /><span>ارسال پاسخ</span></button>
              </div>
            </div>
          </div>
          ...
          <aside className="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="max-h-[520px] overflow-y-auto">
              {filtered.map((t) => {
                ...
                return (
                  <button key={t.id} type="button" onClick={() => setSelectedId(t.id)} className={["flex w-full flex-col items-stretch border-b border-gray-100 px-4 py-3 text-right text-[13px] transition-colors last:border-b-0 dark:border-gray-800", active ? "bg-sky-50/80 dark:bg-sky-900/20" : "hover:bg-gray-50 dark:hover:bg-gray-800/60"].join(" ")}>
                    <div className="flex items-center justify-between gap-2 text-[11px] text-gray-500">
                      <span>{t.shortDate}</span>
                      <span className="inline-flex items-center gap-1 text-[11px] text-gray-400"><MessageCircle size={12} /><span>{t.channel}</span></span>
                    </div>
                    <div className="mt-1 flex items-center justify-between gap-2">
                      <span className="truncate text-[14px] font-semibold text-gray-800 dark:text-gray-100">{t.title}</span>
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-600 dark:bg-gray-800/70 dark:text-gray-300">{t.status}</span>
                    </div>
                    <p className="mt-1 line-clamp-2 text-[12px] leading-5 text-gray-500 dark:text-gray-300">{t.snippet}</p>
                  </button>
                );
              })}
              {filtered.length === 0 && <div className="px-4 py-6 text-center text-[13px] text-gray-500 dark:text-gray-400">تیکتی مطابق فیلتر فعلی پیدا نشد.</div>}
            </div>
          </aside>
        </div>
      </section>
